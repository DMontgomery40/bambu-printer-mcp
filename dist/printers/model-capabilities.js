...
export function isLegacySdcardPrintModel(model) {
...
export function usesRootFtpPrintPath(model, serial) {
    const m = normalizeModelName(model);
    const sn = String(serial ?? "");
    if (P2_MODELS.has(m) || H2_MODELS.has(m) || X2_MODELS.has(m))
        return true;
    if (m && isLegacySdcardPrintModel(m))
        return false;
    if (ROOT_FTP_SERIAL_PREFIXES.some((p) => sn.startsWith(p)))
        return true;
    if (LEGACY_SERIAL_PREFIXES.some((p) => sn.startsWith(p)))
        return false;
    if (m)
        return true;
    return false;
